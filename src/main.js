...
function callback(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    vecs[data[i][0].toLowerCase()] = vec
  }
  if (language === 'de') {
    d3.json('word_selections_de.json', callback1)
  } else {
...
function callback1(errors, rawData) {
  ...
  var wybor = selectDiv.select('#dataSelect').on('change', exampleChange)

  for (var i = 0; i < rawData.length; i++) {
    wybor.append('option').attr('value', rawData[i].id).text(rawData[i].name)
  }
  ...
  modifyDiv.select('#addButton').on('click', function () {
  ...
  modifyDiv.select('#addPairButton').on('click', function () {
  ...
  changeXDiv.select('#Xaxis1').attr('value', currentExample.xAxis[1])
  changeXDiv.select('#Xaxis0').attr('value', currentExample.xAxis[0])

  var changeYDiv = modifyDiv.select('#changeAxesDiv')
  changeYDiv.select('#Yaxis1').attr('value', currentExample.yAxis[1])
  changeYDiv.select('#Yaxis0').attr('value', currentExample.yAxis[0])
  ...
  modifyDiv.select('#changeAxesButton').on('click', function () {
  ...
  function updateExample() {
    ...
    // point deletion
    d3.selectAll('.pointlabel').on('dblclick', function (d) {
      for (var word in currentExample.flat) {
        if (
          currentExample.flat.hasOwnProperty(word) &&
          currentExample.flat[word] == d.group
        ) {
